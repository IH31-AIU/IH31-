            ...
            try
            {
                if (DvItem.Rows[0].Cells[0].Value != null)
                {
                    ...
                    if (int.Parse(price) < int.Parse(dt.Rows[0][0].ToString()))
                    {
                        ...
                        editWord.DocFile = @"C:\ih\word\"+(DvItem.Rows[0].Cells[0]).Value.ToString()+"oder.doc";
                        String pdfPath = @"C:\ih\pdf\"+(DvItem.Rows[0].Cells[0]).Value.ToString()+ "oder.pdf";
                        editWord.TmpFile = @"C:\ih\tml\oder.doc";//テンプレファイル
